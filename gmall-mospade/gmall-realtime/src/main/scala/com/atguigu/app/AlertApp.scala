...
object AlertApp {
  def main(args: Array[String]): Unit = {
    ...
    //4.将KafKa消费到的数据转为样例类,并补全字段,并将数据转为k(mid),v(数据本身)类型的数据
    ...
    //5.开启一个5min的滑动窗口,如果没有设置滑动步长,根据源码中注释可以分析到,默认滑动步长就是批次时间,SparkStreaming默认就是滚动窗口,窗口大小就是批次时间
    ...
    couponAlertInfo.foreachRDD(rdd => {
      rdd.foreachPartition(partition =>{
        ...
        MyEsUtil.insertBulk(GmallConstants.ES_INDEX_ALERT_PREFIX + "2021",list)
      })
    })
    //10.开启任务并阻塞
    ssc.start()
